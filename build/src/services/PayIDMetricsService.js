...
exports.PayIDMetricsService = void 0;
const prom_client_1 = require("prom-client");
class PayIDMetricsService {
    constructor(metricsConfig) {
        this.payIdLookupCounterRegistry = new prom_client_1.Registry();
        this.payIdGaugeRegistry = new prom_client_1.Registry();
        /** Prometheus Counter reporting the number of PayID lookups by [network, environment, org, result]. */
        this.payIdLookupCounter = new prom_client_1.Counter({
        ...
        this.payIdGauge = new prom_client_1.Gauge({
            name: 'payid_count',
    ...
    scheduleRecurringMetricsPush() {
        ...
        const payIdLookupCounterGateway = new prom_client_1.Pushgateway(this.metricsConfig.gatewayUrl, [], this.payIdLookupCounterRegistry);
        const payIdGaugeGateway = new prom_client_1.Pushgateway(this.metricsConfig.gatewayUrl, [], this.payIdGaugeRegistry);
        this.interval = setInterval(() => {
            ...
            payIdLookupCounterGateway.pushAdd({
                ...
                groupings: {
                    instance: `${this.metricsConfig.organization}_${this.metricsConfig.tag}`,
                },
            }, (err, _resp, _body) => {
                if (err) {
            ...
            payIdGaugeGateway.push({
                ...
                groupings: {
                    instance: this.metricsConfig.organization,
                },
            }, (err, _resp, _body) => {
                if (err) {
    ...
    isPushMetricsEnabled() {
        ...
        if (this.metricsConfig.pushIntervalInSeconds <= 0 ||
            Number.isNaN(this.metricsConfig.pushIntervalInSeconds)) {
            console.log(`Invalid PUSH_METRICS_INTERVAL value: ${this.metricsConfig.pushIntervalInSeconds}. Metrics will not be pushed.`);
            return false;
        }
        return true;
    }
     ...
     */
    setPayIdCount(paymentNetwork, environment, count) {
        this.payIdGauge.set({
            paymentNetwork,
            environment,
            org: this.metricsConfig.organization,
     ...
     */
    recordPayIdLookupResult(found, paymentNetwork, environment = 'null') {
        this.payIdLookupCounter.inc({
            paymentNetwork,
            environment,
            org: this.metricsConfig.organization,
...
exports.PayIDMetricsService = PayIDMetricsService;
//# sourceMappingURL=PayIDMetricsService.js.map